...
public class AlbumRepository(ApplicationDbContext context) : Repository<Album>(context), IAlbumRepository
{
    ...
    {
        return await _context.Albums.FirstOrDefaultAsync(album => album.OwnerId == ownerId && album.Title == title, cancellationToken);
    }

    public async Task<IEnumerable<Album>> GetByOwnerIdAndTitleWhereIdNotEqual(Guid ownerId, string title, Guid albumId, CancellationToken cancellationToken)
    {
        return await _context.Albums.Where(album => album.OwnerId == ownerId && album.Title == title && album.Id != albumId).ToListAsync(cancellationToken);
    }
}