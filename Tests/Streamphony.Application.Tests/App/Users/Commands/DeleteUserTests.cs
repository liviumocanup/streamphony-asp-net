...
public class DeleteUserTests
{
    ...
    public async Task Handle_UserExists_DeletesUserAndSongsSuccessfully()
    {
        ...
        _validationServiceMock.Setup(v => v.AssertEntityExists(_unitOfWorkMock.Object.UserRepository, userId, It.IsAny<CancellationToken>(), LogAction.Delete))
                              .Returns(Task.CompletedTask);
        _unitOfWorkMock.Setup(u => u.SongRepository.DeleteWhere(song => song.OwnerId == userId, It.IsAny<CancellationToken>()))
                       .Returns(Task.CompletedTask);
        ...
        _unitOfWorkMock.Verify(u => u.SongRepository.DeleteWhere(It.IsAny<Expression<Func<Song, bool>>>(), CancellationToken.None), Times.Once());
    ...
    public void Handle_UserDoesNotExist_ThrowsNotFoundException()
    {
        ...
        // Act & Assert
        var ex = Assert.ThrowsAsync<NotFoundException>(() => _handler.Handle(deleteUserCommand, CancellationToken.None));
        Assert.That(ex.Message, Is.EqualTo("User not found."));
        _unitOfWorkMock.Verify(u => u.UserRepository.Delete(It.IsAny<Guid>(), It.IsAny<CancellationToken>()), Times.Never());
