...
public class DeleteAlbumTests
{
    ...
    public async Task Handle_AlbumExists_DeletesAlbumAndLogsSuccess()
    {
        ...
        _validationServiceMock.Setup(v => v.AssertEntityExists(_unitOfWorkMock.Object.AlbumRepository, albumId, new CancellationToken(), LogAction.Delete))
    ...
    public void Handle_AlbumDoesNotExist_ThrowsNotFoundException()
    {
        ...
        // Act & Assert
        var ex = Assert.ThrowsAsync<NotFoundException>(() => _handler.Handle(deleteAlbumCommand, CancellationToken.None));
        Assert.That(ex, Is.EqualTo(exception));
        _unitOfWorkMock.Verify(u => u.AlbumRepository.Delete(It.IsAny<Guid>(), It.IsAny<CancellationToken>()), Times.Never());
    }
}